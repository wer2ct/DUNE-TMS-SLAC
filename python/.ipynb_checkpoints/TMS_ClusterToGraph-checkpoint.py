#This script produces labeled graph datasets from dbscan clusters

#Kieran Wall - University of Virginia - August 2025

#Run - python3 ClusterToGraph "edep-file" "dbscan file" "file number" "output directory"
...
def CreateGraphList_Delaunay(graph_points_list_, file_number, occupancy_): #takes an argument of a list of arrays containing the nodes for each cluster properly formatted
...
def main():
    #load in the files we need
    print("initializing")
    DBSCAN_file = np.load(sys.argv[2]) #load dbscan hits
    dbscan_hits = DBSCAN_file['dbscan_hits']
    occupancy_list = DBSCAN_file['dbscan_occupancy']
    const_hit_array = DBSCAN_file['constituents_array']
    ...
    #would be nice to selected what sort of graph we create. This wouldn't be that hard at all to implement. 
    full_file_graph_list = CreateGraphList_Delaunay(graph_points_list, file_number_, occupancy_list)
    print("Completed building graphs")

    #save to our dataset
    file_graph_dataset = FileGraphDataset(root=output_directory, data_list = full_file_graph_list, file_number = file_number_)
    print(f"Graph Dataset for file {file_number_} has been saved to {output_directory}")
